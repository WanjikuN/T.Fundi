import type { CreateProductInput, Product } from "../types/catalog.types";
...
export async function createProduct(
  ...
  if (!input.options) {
    throw new Error("Product characteristics are required.");
  }
  ...
  if (Number.isNaN(input.quantity) || input.quantity < 0) {
    throw new Error("Product quantity must be zero or greater.");
  }
...
export async function updateProduct(
  ...
  const updated: Product = {
    ...
    slug: updates.name !== undefined ? createSlug(updates.name) : current.slug,

    quantity:
      updates.quantity !== undefined ? updates.quantity : current.quantity,

    imageUrl: updates.images?.[0]?.url ?? current.imageUrl,

    images: updates.images?.map((image) => image.url) ?? current.images,
  };
